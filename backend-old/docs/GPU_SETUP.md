...
ASR and TTS runtime settings are hardcoded in `config.py`.
...
so missing CUDA provider support, eSpeak NG, model cache access, and MP3
encoding support fail before the API serves traffic.
