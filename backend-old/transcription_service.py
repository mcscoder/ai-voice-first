...
class _LocalTranscriptionModel:
    def __init__(self) -> None:
        ...
        self.precision = settings.asr.precision.lower()
        self.num_threads = settings.asr.num_threads
        self.provider = settings.asr.provider.strip().lower()
...
class TranscriptionService:
    ...
    async def transcribe(self, file_path: str) -> dict[str, object]:
...
def _run_asr_worker(connection: Connection) -> None:
    ...
    while True:
        ...
        try:
            result = asr.transcribe(job["file_path"])
            connection.send({
                "result": result,
            })
